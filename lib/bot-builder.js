...
module.exports = function botBuilder(
    messageHandler,
    options,
    optionalLogError
) {
    ...
    const apiBuilderOptions = options ? options.apiBuilderOptions : undefined;

    const api = new ApiBuilder(apiBuilderOptions),
        messageHandlerPromise = function(message, originalApiBuilderRequest) {
            return Promise.resolve(message).then(message =>
                messageHandler(message, originalApiBuilderRequest)
            );
        };
